...
namespace Services
{
    public class ProductManager
    {
        ...
        {
            var productList=_context.Products.Include(x => x.Category).Where(x=>!x.IsDeleted).AsQueryable();
            if(categoryId!=null) 
        ...
        {
            return _context.Products.Include(x => x.Category).Where(x => !x.IsMonth)
                .OrderByDescending(x => x.ModifiedOn).Take(count).ToList();
        }
      ...
      public List<Product?> GetByIds(IEnumerable<int> ids)
        ...
        {
            return _context.Products.Where(x => x.IsSlider && !x.IsDeleted).OrderByDescending(x=>x.ModifiedOn).Take(4).ToList();
        }
